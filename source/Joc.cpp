...
Joc::Joc(int llavor, HI::HITexture c[])
{
    a_guanyada=false;
	for(int i=0;i<4;i++){
...
void Joc::mostrar(const point2D &t){
	for (int i = 0; i < 4; i++) {
		if(&a_pals[i]!=nullptr)a_pals[i].Mostrar_Cim_Pila();
	}
	if (&a_ma != nullptr)a_ma.Mostrar_Cim_Pila();
	if (&a_descartades != nullptr)a_descartades.Mostrar_Cim_Pila();
	if (&a_tauler != nullptr)a_tauler.mostrar(t);
}
...
void Joc::Processar_origen(Posicio_Carta origen){ //array de referencies?? //referenccies a punters?
	...
	Carta temp;
	if(origen.alapila && !llista[origen.pila]->buida()) //and llista[origen.pila]->cim().esoberta()
...
bool Joc::PosarAlTauler(Carta car, int col){
    ...
    if(col<=7 && col>0)
        if(a_tauler.Get_Carta_Tauler(col).Casen(car) || (a_tauler.Mida_Col(col)==0 && car.EsRei()))
...
bool Joc::Partida_Guanyada(){
    bool guanyada=true;
    for(int i=0;i<4;i++)
        if(!(a_pals[i].Pila_Completada() && guanyada))guanyada=false;
    return guanyada;
}
...
bool Joc::PosarALaPila(Carta car){
    ...
    condicio1=a_pals[index_pila].buida() && car.EsA();
...
int Joc::Triar_Opcio(Posicio_Carta origen, Posicio_Carta desti){//todo: referencies constants
	...
	if(origen.alapila){//aquet tros és optimitzable pero li deixo la feina al compilador/al futur
		...
		else if(!desti.alapila && origen.pila==1)opcio=2;
		else if(desti.alapila && origen.pila==1 && desti.pila>1)opcio=3;
...
void Joc::Accio(Posicio_Carta origen, Posicio_Carta desti, bool &acciofeta){
	...
	switch(opcio){
		case 1:
			if(a_ma.buida() && !a_descartades.buida())
		...
		case 4:
			...
			if(a_tauler.Mida_Col(col)>0 && (c_origen=a_tauler.Get_Carta_Tauler(col,fila)).esoberta())
				if(a_tauler.Get_Carta_Tauler(col_d).Casen(c_origen) or (c_origen.EsRei() && a_tauler.Mida_Col(col_d)==0)){
		...
		case 5:
			col=origen.columna;
			if(a_tauler.Mida_Col(col)>0 && PosarALaPila(a_tauler.Get_Carta_Tauler(col))){
				a_tauler.Treure_Carta(col);
				a_tauler.Obrir_ultima(col);
			}
...
Posicio_Carta Joc::Localitzar_Carta(point2D t)const{
	Posicio_Carta resultat;
	if(t.y<(margeA+Card_Height+margeB)){
		resultat.alapila=true;
		if(t.x<margeD+Card_Width*2+margeE*2)
			resultat.pila=t.x/(margeD+Card_Width+(margeE/2));
		else{
			const int tpx=320-t.x-(margeD-margeE/2), temp=5-tpx/(margeE+Card_Width);
			resultat.pila=(temp<2?2:temp);
		}
	}
	else{
		resultat.alapila=false;
		int migmarge=margeD/2, tpx=t.x-migmarge, unitat=margeD+Card_Width;
		resultat.columna=(tpx/unitat)+1; //seguim la convencio de la notació de columnes de lusuari i no pas la de les dades
		int tpy=t.y-margeA-Card_Height-margeB, numtancades, numobertes;
